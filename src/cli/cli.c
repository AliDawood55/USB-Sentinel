...
void usbs_cli_print_usage(FILE *stream)
{
    fprintf(stream,
            ...
            "\n"
            "Options for `devices`:\n"
            "  --all      List every volume, not just USB devices (on Windows,\n"
            "             including connected mapped network drives)\n"
            "\n"
            "Usage for `scan`:\n"
            "  usb-sentinel scan [target] [--all] [--signatures <path>]\n"
            "    --all        Let target match any mounted volume - an internal\n"
            "                 disk such as \"C:\", an external HDD, a mapped\n"
            "                 network drive - not only USB devices. Requires a\n"
            "                 target. Locations the OS refuses to open (e.g.\n"
            "                 System Volume Information) are skipped, counted\n"
            "                 in the report, and never end the scan.\n"
